			...
			uint16_t *cpu_cnt)
{
	...
	if (details_ptr->req_node_bitmap) {
		...
		debug("in eval_nodes node_map: %s",str);
		debug3("in+ _eval_nodes, cr_node_cnt: %d",cr_node_cnt);
		for (i=0;i<cr_node_cnt;i++) {
			debug3("in _eval_nodes, cpu_cnt[%d]: %d",i,job_ptr->details->req_node_layout[i]);
			cpu_cnt[i] = job_ptr->details->req_node_layout[i];
		}
		return SLURM_SUCCESS;
	}
				...
				uint16_t cr_type, bool test_only)
{
	...
	/*messagingdebug
	bit_fmt(str,sizeof(str),node_map);
	debug3("after some loop node_map is %s",str);
	*/
	if (bit_set_count(node_map) < min_nodes) {
		xfree(cpu_cnt);
		return NULL;
	...
	/*messagingdebug
	bit_fmt(str,sizeof(str),node_map);
	debug3("in _select_nodes node_map %s",str);
	*/
	...
	/*messagingdebug
	bit_fmt(str,sizeof(str),node_map);
	debug3("in _select_nodes (after _choose_nodes) node_map %s",str);
	*/
	/* if successful, sync up the core_map with the node_map, and
	 * create a cpus array */
	if (rc == SLURM_SUCCESS) {
		/*messagingdebug
		debug("_select_nodes after _choose_nodes slurm_success");
		*/
			...
			struct node_use_record *node_usage)
{
	...
	debug3("in _cr_job_test() bitmap: %s, min_nodes: %d, max_nodes: %d, req_nodes: %d",str,
			min_nodes,max_nodes,req_nodes);
...
alloc_job:
	...
	debug("nhosts: %d",job_res->nhosts);
	debug("number of cpus on job_res %d",job_res->ncpus);
	...
	for (i = 0, n = 0; n < cr_node_cnt; n++) {
		uint32_t j;
		debug("bef cpus on node %d: %d, (n): %d",i,job_res->cpus[i],n);
		...
		debug("cpus on node %d: %d",i,job_res->cpus[i]);
		total_cpus += job_res->cpus[i];
		i++;
	}

	debug("cpus on job_res %d",total_cpus);
/* When 'srun --overcommit' is used, ncpus is set to a minimum value
	...
	if (error_code == SLURM_SUCCESS)
		debug3("in cr_job_test, returning success");
	return error_code;
}

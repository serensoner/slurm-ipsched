/*****************************************************************************\
 *  select_lpconsres.c - select plugin implementation for 
 * 			 linear programming based scheduler plugin.
 *
 ...
 *  Used select/cons_res plugin as base code. There are some functions that are
 *  left untouched. It gets the information of the node/cpu layout DIRECTLY
 *  from sched/lpsched. 
 ...
 */
const char plugin_name[] = "LP-Consumable Resources (CR) Node Selection plugin";
const char plugin_type[] = "select/lpconsres";
...
static void _dump_nodes(void)
{
	...
	for (i=0; i<select_node_cnt; i++) {
		...
		if (gres_list) {
			remaining = gres_plugin_node_state_log(gres_list, node_ptr->name);
		} 
	}
}
		      ...
		      uint32_t req_nodes, uint16_t job_node_req)
{
	int rc;
	char str[64];
	bit_fmt(str,sizeof(str),bitmap);
	debug3("---------------------- in _test_only bitmap: %s, min_nodes: %d, max_nodes: %d, req_nodes: %d",str,
		    ...
		    List preemptee_candidates, List *preemptee_job_list)
{
	...
	orig_map = bit_copy(save_bitmap);
	if (!orig_map)
		fatal("bit_copy: malloc failure");
	bit_fmt(str,sizeof(str),bitmap);
	debug3("---------------------- in _run_now bitmap: %s, min_nodes: %d, max_nodes: %d, req_nodes: %d",str,
			min_nodes,max_nodes,req_nodes);
	rc = cr_job_test(job_ptr, bitmap, min_nodes, max_nodes, req_nodes,
			 ...
			 select_node_usage);
	if (rc == SLURM_SUCCESS)
		debug4("cr_job_test from _run_now has returned success");
			  ...
			  List preemptee_candidates, List *preemptee_job_list)
{
	...
	debug3("---------------------- in _will_run bitmap: %s, min_nodes: %d, max_nodes: %d, req_nodes: %d",str,
			min_nodes,max_nodes,req_nodes);
	rc = cr_job_test(job_ptr, bitmap, min_nodes, max_nodes, req_nodes,
			     ...
			     List *preemptee_job_list)
{
	...
	debug3("before xassert");
	xassert(bitmap);
	debug3("after xassert");
	if (!debug_check) {
		debug_check = true;
...
extern int select_p_select_nodeinfo_set_all(void)
{
	...
	int i=0, n=0, c, start, end, remaining_gpu = 0;
	List gres_list;
	...
	for (n=0; n < select_node_cnt; n++) {
		...
		*/
		...
		if (select_node_usage[n].gres_list)
			...
			gres_list = node_ptr->gres_list;
		if (gres_list) {
			remaining_gpu = gres_plugin_node_state_log(gres_list, node_ptr->name);
		} 
		...
		nodeinfo->rem_cpus = select_node_record[n].cpus - nodeinfo->alloc_cpus;
		nodeinfo->alloc_mem = select_node_usage[n].alloc_memory;
		nodeinfo->rem_gpus = remaining_gpu;
	}
...
int print_nodes_lpconsres(struct node_record *node_ptr, int node_cnt)
{
	select_p_node_init(node_record_table_ptr, node_cnt);
	return SLURM_SUCCESS;
}
